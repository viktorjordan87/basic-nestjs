...
import { CustomLoggerNestjsService } from '@common/logging';
...
export class VersioningHeader2Service {
  ...
  private readonly customLogger = new CustomLoggerNestjsService('test-nestjs');
  ...
  findAll(schedulerRegistry: SchedulerRegistry) {
    ...
    this.customLogger.error(
      `findAll: This action returns all versioningHeader2 V3 + V4 + and execute cron task ${CRONTASKS_NAMES.HANDLE_CRON4} + next execution date: ${nextExecutionDate.toISO()}`,
    );
